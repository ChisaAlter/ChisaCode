...
seed_worktree_chisacode_home() {
  local source_home="${CHISACODE_DEV_SEED_HOME:-$HOME/.chisacode}"
  local target_home="$1"
  ...
  if [ "${CHISACODE_DEV_RESET_HOME:-0}" = "1" ]; then
    rm -rf "$target_home"
...
configure_dev_chisacode_home() {
  if [ -n "${CHISACODE_HOME:-}" ]; then
    export CHISACODE_HOME
    return
  fi
  ...
  if [ -n "$git_dir" ] && [ -n "$git_common_dir" ] && [ "$git_dir" != "$git_common_dir" ]; then
    ...
    CHISACODE_HOME="$HOME/.chisacode-${worktree_name}"
