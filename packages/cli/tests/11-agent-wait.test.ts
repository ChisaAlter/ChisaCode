...
try {
  ...
  // Test 2: wait requires id argument
  {
    ...
    const result =
      await $`CHISACODE_HOST=localhost:${port} CHISACODE_HOME=${chisacodeHome} npx chisacode wait`.nothrow();
  ...
  // Test 3: wait handles daemon not running
  {
    ...
    const result =
      await $`CHISACODE_HOST=localhost:${port} CHISACODE_HOME=${chisacodeHome} npx chisacode wait abc123`.nothrow();
  ...
  // Test 4: wait --timeout flag is accepted
  {
    ...
    const result =
      await $`CHISACODE_HOME=${chisacodeHome} npx chisacode wait --timeout 30 --host localhost:${port} abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 5: wait --host flag is accepted
  {
    ...
    const result =
      await $`CHISACODE_HOME=${chisacodeHome} npx chisacode wait --host localhost:${port} abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 6: -q (quiet) flag is accepted with wait
  {
    ...
    const result =
      await $`CHISACODE_HOST=localhost:${port} CHISACODE_HOME=${chisacodeHome} npx chisacode -q wait abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 7: --json flag is accepted with wait
  {
    ...
    const result =
      await $`CHISACODE_HOST=localhost:${port} CHISACODE_HOME=${chisacodeHome} npx chisacode wait abc123 --json`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 8: --format yaml flag is accepted with wait
  {
    ...
    const result =
      await $`CHISACODE_HOST=localhost:${port} CHISACODE_HOME=${chisacodeHome} npx chisacode --format yaml wait abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 9: chisacode --help shows wait command
  {
    console.log("Test 9: chisacode --help shows wait command");
    ...
    console.log("chisacode --help shows wait command\n");
  }
