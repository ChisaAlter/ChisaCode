...
async function setup(): Promise<void> {
  ...
  try {
    ...
    console.log(`CHISACODE_HOME: ${ctx.chisacodeHome}`);
...
async function test_wait_for_initial_task(agentId: string): Promise<void> {
  ...
  const result = await ctx.chisacode(["wait", "--timeout", "120s", agentId], { timeout: 130000 });
...
async function test_agent_send(agentId: string): Promise<void> {
  ...
  const result = await ctx.chisacode(["send", agentId, 'Now say "follow-up task complete"'], {
    timeout: 180000,
  });
...
async function test_verify_logs(agentId: string): Promise<void> {
  ...
  const result = await ctx.chisacode(["logs", "--tail", "50", agentId]);
...
async function test_agent_delete(agentId: string): Promise<void> {
  ...
  const result = await ctx.chisacode(["delete", agentId]);
