...
import { parseConnectionOfferFromUrl } from "@chisacode/protocol/connection-offer";
import { generateLocalPairingOffer } from "@chisacode/server";
...
(shouldRunRelayE2e ? describe : describe.skip)("CLI --host offer URL via relay", () => {
  ...
  beforeAll(async () => {
    ...
    ctx = await createE2ETestContext({
      timeout: 60_000,
      env: {
        CHISACODE_RELAY_ENABLED: "true",
        CHISACODE_RELAY_ENDPOINT: relayEndpoint,
        CHISACODE_RELAY_PUBLIC_ENDPOINT: relayEndpoint,
      },
    });
  ...
  it("runs `chisacode --host <offer-url> ls` over the relay and matches direct ls output", async () => {
    ...
    const direct = await ctx.chisacode(["ls", "--json"]);
