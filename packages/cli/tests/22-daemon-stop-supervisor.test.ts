...
/**
 * Regression: `chisacode daemon stop` must stop supervised dev daemons
...
async function readPidLockState(chisacodeHome: string): Promise<PidLockState> {
  const pidPath = join(chisacodeHome, "chisacode.pid");
...
const chisacodeHome = await mkdtemp(join(tmpdir(), "chisacode-stop-supervisor-"));
...
try {
  ...
  const statusBeforeStop = await readDaemonStatus(chisacodeHome);
  ...
  const pidLockBeforeStop = await readPidLockState(chisacodeHome);
  ...
  console.log("Test 2: `chisacode daemon stop` should stop without respawn");
  const stopResult =
    await $`CHISACODE_HOME=${chisacodeHome} CHISACODE_LOCAL_SPEECH_AUTO_DOWNLOAD=${testEnv.CHISACODE_LOCAL_SPEECH_AUTO_DOWNLOAD} CHISACODE_DICTATION_ENABLED=${testEnv.CHISACODE_DICTATION_ENABLED} CHISACODE_VOICE_MODE_ENABLED=${testEnv.CHISACODE_VOICE_MODE_ENABLED} npx chisacode daemon stop --home ${chisacodeHome} --json`.nothrow();
  ...
  const pidAfterStop = await readPidLockState(chisacodeHome);
  ...
  const statusAfterStop = await readDaemonStatus(chisacodeHome);
  assert.strictEqual(
