...
try {
  // Test 1: permit --help shows subcommands
  {
    ...
    const result = await $`npx chisacode permit --help`.nothrow();
  ...
  // Test 2: permit ls --help shows options
  {
    ...
    const result = await $`npx chisacode permit ls --help`.nothrow();
  ...
  // Test 3: permit ls returns error when no daemon running
  {
    ...
    const result =
      await $`CHISACODE_HOST=localhost:${port} CHISACODE_HOME=${chisacodeHome} npx chisacode permit ls`.nothrow();
  ...
  // Test 4: permit ls --json handles errors
  {
    ...
    const result =
      await $`CHISACODE_HOST=localhost:${port} CHISACODE_HOME=${chisacodeHome} npx chisacode permit ls --json`.nothrow();
  ...
  // Test 5: -q (quiet) flag is accepted globally
  {
    ...
    const result =
      await $`CHISACODE_HOST=localhost:${port} CHISACODE_HOME=${chisacodeHome} npx chisacode -q permit ls`.nothrow();
    const output = result.stdout + result.stderr;
