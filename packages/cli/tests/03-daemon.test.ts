 ...
 *
 * Tests daemon commands with an isolated CHISACODE_HOME.
 *
 * Tests:
...
try {
  // Test 1: daemon --help shows subcommands
  {
    ...
    const result = await runLocalChisaCode(["daemon", "--help"]);
  ...
  // Test 5: daemon status --json outputs valid JSON
  {
    ...
    assert.strictEqual(status.home, chisacodeHome, "json status should reflect the isolated home");
    assert.strictEqual(
      status.hostname,
      null,
