...
async function waitForLoopInList(
  ...
  for (let attempt = 0; attempt < 20; attempt++) {
    const listed = await ctx.chisacode(["loop", "ls", "--json"]);
...
try {
  {
    ...
    const created = await ctx.chisacode(
      [
        "schedule",
        "create",
        "Review new PRs",
    ...
    const listed = await ctx.chisacode(["schedule", "ls", "--json"]);
    assert.strictEqual(listed.exitCode, 0, listed.stderr);
    const listedJson = JSON.parse(listed.stdout);
    assert(Array.isArray(listedJson), listed.stdout);
    assert(
    ...
    const inspected = await ctx.chisacode(["schedule", "inspect", createdJson.id, "--json"]);
...

  {
    ...
    const result = await ctx.chisacode(
      [
        "schedule",
        "create",
...

  {
    ...
    async function pollStatus(attempt: number): Promise<string> {
      ...
      const inspect = await ctx.chisacode(["loop", "inspect", runJson.id, "--json"]);
    ...
    const logs = await ctx.chisacode(["loop", "logs", runJson.id], { timeout: 15000 });
    assert.strictEqual(logs.exitCode, 0, logs.stderr);
    assert(logs.stdout.includes("verify-check"), logs.stdout);

    const stopped = await ctx.chisacode(["loop", "stop", runJson.id, "--json"]);
