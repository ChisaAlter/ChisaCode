...
async function connectClient(): Promise<{ client: ChisaCodeClient; ws: FakeWebSocket }> {
  ...
  expect(hello.clientId).toEqual(expect.stringMatching(/^chisacode-sdk-/));
  ws.message(
    sessionMessage({
      type: "status",
      payload: {
...
function createWorkspace(input: Partial<ChisaCodeWorkspace> = {}): ChisaCodeWorkspace {
  return {
    id: "workspace_sdk",
...
function createAgent(input: Partial<ChisaCodeAgent> = {}): ChisaCodeAgent {
  return {
    id: "agent_sdk",
    provider: "codex",
    cwd: "/repo/sdk",
...
test("createChisaCodeClient exposes workspace list through the daemon client", async () => {
