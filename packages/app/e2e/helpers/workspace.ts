...
export const createTempGitRepo = async (
  ...
  execSync('git config user.email "e2e@chisacode.test"', { cwd: repoPath, stdio: "ignore" });
  execSync('git config user.name "ChisaCode E2E"', { cwd: repoPath, stdio: "ignore" });
  execSync("git config commit.gpgsign false", { cwd: repoPath, stdio: "ignore" });
  await writeFile(path.join(repoPath, "README.md"), "# Temp Repo\n");
  if (options?.chisacodeConfig) {
    await writeFile(
      path.join(repoPath, "chisacode.json"),
  ...
  if (options?.chisacodeConfig) {
    execSync("git add chisacode.json", { cwd: repoPath, stdio: "ignore" });
  }
  for (const file of options?.files ?? []) {
  ...
  for (const branch of branches) {
    ...
    const markerPath = `.chisacode-e2e-${branch.replace(/[^a-zA-Z0-9._-]/g, "-")}.txt`;
 ...
 */
export async function createTempDirectory(prefix = "chisacode-e2e-dir-"): Promise<TempDirectory> {
