...
export async function measureKeystrokeLatency(page: Page, char: string): Promise<number> {
  await page.evaluate(() => {
    const win = window as Window & {
      __chisacodeTerminal?: { onWriteParsed: (cb: () => void) => { dispose: () => void } };
      __perfKeystroke?: { promise: Promise<number> | null };
    };
    if (!win.__chisacodeTerminal) {
      throw new Error("__chisacodeTerminal not available");
    }
    const term = win.__chisacodeTerminal;
