...
function makeOffer(input?: Partial<ConnectionOffer>): ConnectionOffer {
  return {
    ...
    relay: {
      endpoint: input?.relay?.endpoint ?? "relay.chisacode.sh:443",
...
function encodeOfferUrl(payload: unknown): string {
  const encoded = Buffer.from(JSON.stringify(payload), "utf8")
    ...
    .replace(/\//g, "_")
    .replace(/=+$/g, "");
  return `https://app.chisacode.sh/#offer=${encoded}`;
}
...
describe("HostRuntimeController", () => {
  it("replaces the active relay client when re-pairing changes the daemon public key", async () => {
    const oldRelay: HostConnection = {
      id: "relay:wss:relay.chisacode.sh:443",
      type: "relay",
      relayEndpoint: "relay.chisacode.sh:443",
      useTls: true,
  ...
  it("adopts the first successful probe on startup", async () => {
    ...
    const latencies: Record<string, number | Error> = {
      "direct:lan:6767": 82,
      "relay:relay.chisacode.sh:443": 18,
    };
    const controller = new HostRuntimeController({
      host,
    ...
    const controller = new HostRuntimeController({
      ...
      deps: {
        ...
        connectToDaemon: async ({ host: hostProfile, connection }) => {
          ...
          if (connection.id === "relay:relay.chisacode.sh:443") {
  ...
  it("probes the active online connection through the existing client", async () => {
    ...
    const latencies: Record<string, number | Error> = {
      "direct:lan:6767": 12,
      "relay:relay.chisacode.sh:443": 65,
    };
    const controller = new HostRuntimeController({
      host,
      deps: {
    ...
    expect(probeAttempts).toEqual(["relay:relay.chisacode.sh:443"]);
    expect(controller.getSnapshot().activeConnectionId).toBe("direct:lan:6767");
    expect(controller.getSnapshot().connectionStatus).toBe("online");
  ...
  it("fails over when the active client ping fails", async () => {
    ...
    const latencies: Record<string, number | Error> = {
      "direct:lan:6767": 15,
      "relay:relay.chisacode.sh:443": 55,
    };
    const controller = new HostRuntimeController({
      host,
      deps: makeDeps(latencies, clients),
    ...
    latencies["relay:relay.chisacode.sh:443"] = 42;
    clearProbeBackoff(controller);
    await controller.runProbeCycleNow();

    const snapshot = controller.getSnapshot();
    expect(snapshot.activeConnectionId).toBe("relay:relay.chisacode.sh:443");
    expect(snapshot.connectionStatus).toBe("online");
  ...
  it("backs off inactive connection probes while a host is online", async () => {
    ...
    const latencies: Record<string, number | Error> = {
      "direct:lan:6767": 10,
      "relay:relay.chisacode.sh:443": 50,
    };
    const controller = new HostRuntimeController({
      host,
      deps: makeDeps(latencies, clients),
    ...
    const initialRelayProbe = controller
      .getSnapshot()
      .probeByConnectionId.get("relay:relay.chisacode.sh:443");

    latencies["direct:lan:6767"] = 12;
    latencies["relay:relay.chisacode.sh:443"] = 25;
    const lastProbedAt = (
      controller as unknown as {
        connectionLastProbedAt: Map<string, number>;
      }
    ...
    lastProbedAt.set("relay:relay.chisacode.sh:443", now - 60_000);
    ...
    expect(snapshot.probeByConnectionId.get("relay:relay.chisacode.sh:443")).toEqual(
      initialRelayProbe,
    );
  });
  ...
  it("switches only after the faster alternative wins consecutive probes", async () => {
    ...
    const latencies: Record<string, number | Error> = {
      "direct:lan:6767": 15,
      "relay:relay.chisacode.sh:443": 60,
    };
    const controller = new HostRuntimeController({
      host,
      deps: makeDeps(latencies, clients),
    ...
    latencies["relay:relay.chisacode.sh:443"] = 30;
    clearProbeBackoff(controller);
    await controller.runProbeCycleNow();
    expect(controller.getSnapshot().activeConnectionId).toBe("direct:lan:6767");
    ...
    let switched = controller.getSnapshot().activeConnectionId === "relay:relay.chisacode.sh:443";
    for (let index = 0; index < 6 && !switched; index += 1) {
      clearProbeBackoff(controller);
      await controller.runProbeCycleNow();
      switched = controller.getSnapshot().activeConnectionId === "relay:relay.chisacode.sh:443";
    }
  ...
  it("does not switch on a transient latency spike", async () => {
    ...
    const latencies: Record<string, number | Error> = {
      "direct:lan:6767": 15,
      "relay:relay.chisacode.sh:443": 80,
    };
    const controller = new HostRuntimeController({
      host,
      deps: makeDeps(latencies, clients),
    ...
    latencies["relay:relay.chisacode.sh:443"] = 20;
    clearProbeBackoff(controller);
    await controller.runProbeCycleNow();
    expect(controller.getSnapshot().activeConnectionId).toBe("direct:lan:6767");

    latencies["direct:lan:6767"] = 20;
    latencies["relay:relay.chisacode.sh:443"] = 90;
    clearProbeBackoff(controller);
    await controller.runProbeCycleNow();
    expect(controller.getSnapshot().activeConnectionId).toBe("direct:lan:6767");
  ...
  it("ignores stale switch failures after a newer connection is already online", async () => {
    ...
    ).switchToConnection({ connectionId: "relay:relay.chisacode.sh:443" });
    await waitUntil(() => {
      const snapshot = controller.getSnapshot();
      return (
        snapshot.activeConnectionId === "relay:relay.chisacode.sh:443" &&
        snapshot.connectionStatus === "online"
      );
    });
  ...
  it("notifies once and replaces the probe map when probe contents change", () => {
    ...
    const changedProbeMap = makeProbeMap([
      ["direct:lan:6767", { status: "available", latencyMs: 12 }],
      ["relay:relay.chisacode.sh:443", { status: "unavailable", latencyMs: null }],
    ]);
...
describe("HostRuntimeStore", () => {
      ...
      makeFetchAgentsPayload({
        entries: [
          makeFetchAgentsEntry({
            id: "agent-recent",
            cwd: "/Users/moboudra/dev/chisacode",
      ...
      makeFetchAgentsPayload({
        entries: [
          makeFetchAgentsEntry({
            ...
            cwd: "/Users/moboudra/dev/chisacode-pr67-review",
  ...
  it("upsertDirectConnection stores SSL and password settings", async () => {
    ...
    await store.upsertDirectConnection({
      ...
      endpoint: "example.chisacode.test:7443",
      useTls: true,
    ...
    expect(host?.connections).toEqual([
      {
        id: "direct:example.chisacode.test:7443",
        type: "directTcp",
        endpoint: "example.chisacode.test:7443",
        useTls: true,
        password: "shared-secret",
