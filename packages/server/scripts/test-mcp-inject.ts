...
import { createTestChisaCodeDaemon } from "../src/server/test-utils/chisacode-daemon.js";
...
async function verifyInjectedMcpForProvider(
  ...
  try {
    const prompt = [
      ...
      "If you have a tool called list_agents or create_agent from a chisacode MCP server, call list_agents once.",
      "After checking, reply with exactly CHISACODE_MCP_FOUND.",
      "If you do not have those tools, reply with exactly CHISACODE_MCP_NOT_FOUND.",
    ...
    if (!assistantText.includes("CHISACODE_MCP_FOUND")) {
      throw new Error(
        `Expected assistant to confirm ChisaCode MCP availability. Assistant text:\n${assistantText}`,
      );
    }
    ...
    const listAgentsCalls = toolCalls.filter(
      (call) =>
        ...
        call.name === "chisacode.list_agents" ||
...
async function main(): Promise<void> {
  ...
  const rootCwd = await mkdtemp(path.join(os.tmpdir(), "chisacode-mcp-inject-real-"));
