...
function buildTerminalTimelineItem(input: {
  ...
  if (input.status === "running") {
    return {
      type: "tool_call",
      name: "chisacode_worktree_terminals",
      callId: input.callId,
      status: "running",
      detail: {
        type: "unknown",
...
export async function spawnWorkspaceScript(
  ...
  const configResult = readChisaCodeConfig(repoRoot);
  if (!configResult.ok) {
    throw chisacodeConfigParseError(configResult);
  }
  const scriptConfigs = getScriptConfigs(configResult.config);
  const config = scriptConfigs.get(scriptName);
  if (!config) {
    throw new Error(`Script '${scriptName}' is not configured in chisacode.json`);
  }
