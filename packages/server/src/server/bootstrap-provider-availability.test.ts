...
describe("bootstrap provider availability", () => {
  ...
  test("loads a persisted Codex record without spawning a missing Codex binary", async () => {
    const { createChisaCodeDaemon } = await import("./bootstrap.js");
    const root = await mkdtemp(path.join(os.tmpdir(), "chisacode-bootstrap-provider-"));
    tempRoots.push(root);
    const binDir = await mkdtemp(path.join(os.tmpdir(), "chisacode-bootstrap-provider-bin-"));
    ...
    const agentStoragePath = path.join(chisacodeHome, "agents");
    ...
    const daemon = await createChisaCodeDaemon(config, pino({ level: "silent" }));
    try {
