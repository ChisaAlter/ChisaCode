...
describe("resolveDaemonVersion", () => {
  it("resolves server version by walking up to @fleurdelys/server package.json", () => {
    const root = createTempDir();
    writeFileSync(
      path.join(root, "package.json"),
      JSON.stringify({ name: "@fleurdelys/server", version: "9.8.7" }),
      "utf8",
    );
  ...
  it("throws when @fleurdelys/server package metadata cannot be resolved", () => {
    const root = createTempDir();
    writeFileSync(
      path.join(root, "package.json"),
  ...
  it("throws when @fleurdelys/server version is missing", () => {
    const root = createTempDir();
    writeFileSync(
      path.join(root, "package.json"),
      JSON.stringify({ name: "@fleurdelys/server" }),
      "utf8",
    );
    const nestedDir = path.join(root, "dist", "server");
    mkdirSync(nestedDir, { recursive: true });
