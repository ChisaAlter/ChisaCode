...
test("logs redacted query summary and never leaks sentinel secrets", async () => {
  const envSecret = "CHISACODE_ENV_SENTINEL_SECRET";
  const runtimeSecret = "CHISACODE_RUNTIME_SENTINEL_SECRET";
  const systemSecret = "CHISACODE_SYSTEM_PROMPT_SENTINEL_SECRET";
  const previousEnv = process.env.CHISACODE_TEST_SENTINEL_SECRET;
  process.env.CHISACODE_TEST_SENTINEL_SECRET = envSecret;
  ...
  const client = new ClaudeAgentClient({
    ...
    runtimeSettings: {
      env: {
        CHISACODE_RUNTIME_SENTINEL_SECRET: runtimeSecret,
      },
    },
    resolveBinary: async () => "/test/claude/bin",
  });
  ...
  try {
    ...
    if (previousEnv === undefined) {
      delete process.env.CHISACODE_TEST_SENTINEL_SECRET;
    } else {
      process.env.CHISACODE_TEST_SENTINEL_SECRET = previousEnv;
    }
  }
});
