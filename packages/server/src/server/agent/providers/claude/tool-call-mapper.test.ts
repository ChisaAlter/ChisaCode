  ...
  it("maps Glob calls as search detail using pattern input", () => {
    const item = expectMapped(
      mapClaudeCompletedToolCall({
        ...
        name: "Glob",
        input: { pattern: "**/.claude/commands/chisacode*" },
        output: {
          durationMs: 7,
          numFiles: 2,
    ...
    expect(item.detail).toEqual({
      type: "search",
      query: "**/.claude/commands/chisacode*",
      toolName: "glob",
  ...
  it("maps Grep calls as search detail using pattern input", () => {
    const item = expectMapped(
      mapClaudeCompletedToolCall({
        ...
        input: {
          pattern: '\\\\\\"cli\\\\\\""',
          path: "/Users/moboudra/dev/chisacode/packages/desktop/src",
          output_mode: "content",
          "-n": true,
        },
        output: {
  ...
  it("normalizes namespaced voice MCP speak tools", () => {
    const item = expectMapped(
      mapClaudeCompletedToolCall({
        ...
        name: "mcp__chisacode_voice__speak",
