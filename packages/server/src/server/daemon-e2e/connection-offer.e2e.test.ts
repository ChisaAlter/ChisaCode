...
async function getPairingOfferUrl(args: {
  ...
  const pairing = await generateLocalPairingOffer({
    chisacodeHome: args.chisacodeHome,
...
describe("ConnectionOfferV2 (daemon E2E)", () => {
  ...
  test("emits relay-only offer URL with stable serverId", async () => {
    ...
    try {
      const offerUrl = await getPairingOfferUrl({
        chisacodeHome: daemon.chisacodeHome,
      ...
      expect(offerUrl.startsWith("https://app.chisacode.sh/#offer=")).toBe(true);
      ...
      expect(offer.relay.endpoint).toBe("relay.chisacode.sh:443");
  ...
  test("persists serverId and daemon keypair across daemon restarts", async () => {
    ...
    const tempHomeRoot = await mkdtemp(path.join(os.tmpdir(), "chisacode-offer-home-"));

    const { logger: logger1 } = createCapturingLogger();
    const daemon1 = await createTestChisaCodeDaemon({
      listen: "0.0.0.0",
      logger: logger1,
      relayEnabled: true,
    ...
    try {
      const offerUrl1 = await getPairingOfferUrl({
        chisacodeHome: daemon1.chisacodeHome,
      ...
      const daemon2 = await createTestChisaCodeDaemon({
        listen: "0.0.0.0",
        logger: logger2,
        relayEnabled: true,
        chisacodeHomeRoot: tempHomeRoot,
      ...
      try {
        const offerUrl2 = await getPairingOfferUrl({
          chisacodeHome: daemon2.chisacodeHome,
  ...
  test("respects --no-relay (CLI) by not emitting a pairing offer", async () => {
    ...
    const tempHome = await mkdtemp(path.join(os.tmpdir(), "chisacode-offer-e2e-"));
