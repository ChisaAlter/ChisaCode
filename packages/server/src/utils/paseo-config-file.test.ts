...
describe("paseo config file substrate", () => {
  ...
  it("returns invalid_project_config for invalid fleurdelys.json", () => {
    writeFileSync(join(tempDir, "fleurdelys.json"), "{ invalid json\n");
  ...
  it("falls back to legacy paseo.json when fleurdelys.json is missing", () => {
    writeFileSync(join(tempDir, "paseo.json"), JSON.stringify({ worktree: { setup: "npm ci" } }));
  ...
  it("prefers fleurdelys.json when both config files exist", () => {
    writeFileSync(join(tempDir, "paseo.json"), JSON.stringify({ worktree: { setup: "old" } }));
    writeFileSync(join(tempDir, "fleurdelys.json"), JSON.stringify({ worktree: { setup: "new" } }));
  ...
  it("writes pretty JSON with a trailing newline when revision matches", () => {
    writeFileSync(join(tempDir, "fleurdelys.json"), JSON.stringify({ worktree: { setup: "old" } }));
    ...
    expect(readFileSync(join(tempDir, "fleurdelys.json"), "utf8")).toBe(
      '{\n  "worktree": {\n    "setup": "npm install"\n  }\n}\n',
    );
  });

  it("writes legacy paseo.json when it is the only existing config file", () => {
    writeFileSync(join(tempDir, "paseo.json"), JSON.stringify({ worktree: { setup: "old" } }));
    const expectedRevision = statPaseoConfigPath(tempDir);
  ...
  it("creates fleurdelys.json when the file is still missing and expected revision is null", () => {
    ...
    expect(readFileSync(join(tempDir, "nested", "fleurdelys.json"), "utf8")).toBe(
      '{\n  "scripts": {\n    "dev": {\n      "command": "npm run dev"\n    }\n  }\n}\n',
    );
  });
});
