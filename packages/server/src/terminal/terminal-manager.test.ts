...
it("inherits registered env for the worktree root cwd", async () => {
  ...
  manager.registerCwdEnv({
    cwd,
    env: { CHISACODE_WORKTREE_PORT: "45678" },
  });
  await manager.createTerminal({
    ...
    args: [
      "-e",
      `require('fs').writeFileSync(${JSON.stringify(markerPath)}, process.env.CHISACODE_WORKTREE_PORT ?? '')`,
    ],
  });
...
it("inherits registered env for subdirectories within the worktree", async () => {
  ...
  manager.registerCwdEnv({
    cwd: rootCwd,
    env: { CHISACODE_WORKTREE_PORT: "45679" },
  });
  await manager.createTerminal({
    cwd: subdirCwd,
    command: process.execPath,
