...
set "APP_EXECUTABLE=%RESOURCES_DIR%\..\Fleurdelys.exe"
if not exist "%APP_EXECUTABLE%" (
  set "APP_EXECUTABLE=%RESOURCES_DIR%\..\Paseo.exe"
  if not exist "%APP_EXECUTABLE%" (
    echo Bundled Fleurdelys executable not found at %APP_EXECUTABLE% 1>&2
    exit /b 1
  )
)
