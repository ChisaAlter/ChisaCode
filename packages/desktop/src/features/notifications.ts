...
export function registerNotificationHandlers(): void {
  ...
  const send = async (event: Electron.IpcMainInvokeEvent, rawInput?: NotificationInput) => {
    ...
    notification.on("click", () => {
      ...
      if (win && data && Object.keys(data).length > 0) {
        ...
        for (const prefix of IPC_PREFIXES) {
          win.webContents.send(`${prefix}:event:notification-click`, payload);
        }
      }
  ...
  for (const prefix of IPC_PREFIXES) {
    ipcMain.handle(`${prefix}:notification:isSupported`, isSupported);
    ipcMain.handle(`${prefix}:notification:send`, send);
  }
}
