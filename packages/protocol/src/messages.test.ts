...
describe("usage statistics protocol", () => {
  test("parses usage RPC requests with compatible defaults", () => {
    expect(
      SessionInboundMessageSchema.parse({
  ...
  test("parses usage summary, export, and clear responses", () => {
    const summary = SessionOutboundMessageSchema.parse({
    ...
    expect(summary.type).toBe("usage.summary.get.response");
