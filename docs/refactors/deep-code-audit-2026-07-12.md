...
| -------- | -------: | --------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| 架构设计 |      9.9 | dependency-cruiser 0 违规；protocol 2164 行；workspace 2592 行；Claude 873 行；ACP 926 行；Pi 581 行      | workspace header/center-column render shell 仍集中在主屏                                        |
...
4. **P2 app 工作台拆分（进行中）**：移动端 navigation、workspace command routing、layout/setup persistence/hydration、tab/pane/dock/content、environment panel state/data/view、explorer 与 open-intent 已提取，`workspace-screen.tsx` 从 5453 降至 2592 行；下一步评估 header/center-column render shell，保持 native/web/electron surface 测试分离。
