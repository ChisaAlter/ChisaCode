...
| -------- | -------: | --------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| 架构设计 |      9.3 | dependency-cruiser 0 违规；DaemonClient 领域、request 与 transport/reconnect authority 已分离，核心降至 2319 行 | provider adapters、`workspace-screen.tsx`、`messages.ts` 仍是 4k+ 行责任中心                    |
...
| 代码质量 |      9.1 | typecheck/lint/format/test-audit/高信号 Knip/依赖边界均有门禁；client 请求与连接状态机均具备独立行为契约        | 历史 test debt 高，核心测试文件超过 5k 行，完整 Knip unused-export 结果仍有大量噪声与真实债混合 |
