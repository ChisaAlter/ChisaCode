...
| -------- | -------: | --------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| 架构设计 |      9.8 | dependency-cruiser 0 违规；protocol 2164 行；workspace 3970 行；Claude 873 行；ACP 926 行；Pi 581 行      | workspace pane move/reorder 与 dock orchestration 仍集中在主屏                                  |
...
4. **P2 app 工作台拆分（进行中）**：移动端 navigation、workspace command routing、layout/setup persistence/hydration、tab open/create/navigation 与 close/bulk-close actions 已提取，`workspace-screen.tsx` 从 5453 降至 3970 行；下一步评估 pane move/reorder 与 dock orchestration，保持 native/web/electron surface 测试分离。
