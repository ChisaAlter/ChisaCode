...
| -------- | -------: | ----------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| 架构设计 |      9.8 | dependency-cruiser 0 违规；protocol 主文件 2164 行；workspace mobile tab 导航提取后主屏从 5453 降至 4926 行 | provider adapters 与 workspace 命令/持久化编排仍是主要责任中心                                  |
...
2. **P1 provider 文件拆分**：`providers/base/` 错误抽象已删除，Codex/Claude/OpenCode 主路径已按 composition-first 拆分；下一步收敛 `acp-agent.ts`、`pi/agent.ts` 与 Claude Session 的剩余大职责。
