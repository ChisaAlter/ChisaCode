...
| -------- | -------: | -------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| 架构设计 |      9.2 | dependency-cruiser 0 违规；DaemonClient 领域客户端与 request/waiter authority 已分离，核心降至 3040 行   | provider adapters、`workspace-screen.tsx`、`messages.ts` 仍是 4k+ 行责任中心                    |
...
| 代码质量 |      9.0 | typecheck/lint/format/test-audit/高信号 Knip/依赖边界均有门禁；client 请求状态机具备独立行为契约         | 历史 test debt 高，核心测试文件超过 5k 行，完整 Knip unused-export 结果仍有大量噪声与真实债混合 |
| 综合     |  **9.2** | 核心安全、主要产品域 parity 与持续领域拆分均有代码级实现和精确验证                                       | 继续提升需要完成 provider/app/protocol 大模块治理和依赖专项                                     |
...
3. **P1 client/protocol 拆分（进行中）**：`daemon-client.ts` 已完成文件传输、checkout、管理命令、automation、workspace、terminal、voice/dictation、agent lifecycle/config、interaction/query 与 request/waiter authority 分域，核心降至 3040 行；下一步拆 transport/reconnect 连接生命周期，并按 RPC domain 拆 `messages.ts`。
