...
| -------- | -------: | --------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| 架构设计 |      9.8 | dependency-cruiser 0 违规；protocol 主文件 2164 行；workspace 主屏 4926 行；ACP provider 主文件 2446 行   | provider adapters 与 workspace 命令/持久化编排仍是主要责任中心                                  |
...
- 2026-07-13：ACP 增量 tool snapshot、plan/timeline/detail、permission request/option 与 raw payload 解析提取到 `acp/tool-call-mapper.ts`；Session 保留状态和 terminal 生命周期，原类型入口兼容重导出，主文件从 2860 降至 2446 行。
...
2. **P1 provider 文件拆分**：`providers/base/` 错误抽象已删除，Codex/Claude/OpenCode 主路径已按 composition-first 拆分，ACP tool/permission mapper 已独立；下一步收敛 ACP 剩余 runtime/session 编排、`pi/agent.ts` 与 Claude Session 的剩余大职责。
