...
| -------- | -------: | --------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| 架构设计 |      9.8 | dependency-cruiser 0 违规；protocol 2164 行；workspace 4451 行；Claude 873 行；ACP 926 行；Pi 581 行      | workspace tab lifecycle/open-file orchestration 仍是主要责任中心                                |
...
4. **P2 app 工作台拆分（进行中）**：移动端 navigation、workspace command routing 与 layout/setup persistence/hydration 已提取，`workspace-screen.tsx` 从 5453 降至 4451 行；下一步评估 tab lifecycle/open-file orchestration，保持 native/web/electron surface 测试分离。
