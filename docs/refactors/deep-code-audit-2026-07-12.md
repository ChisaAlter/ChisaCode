...
| -------- | -------: | --------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| 架构设计 |      9.8 | dependency-cruiser 0 违规；protocol 2164 行；workspace 4270 行；Claude 873 行；ACP 926 行；Pi 581 行      | workspace tab close/bulk-close lifecycle 仍是主要责任中心                                       |
...
4. **P2 app 工作台拆分（进行中）**：移动端 navigation、workspace command routing、layout/setup persistence/hydration 与 tab open/create/navigation actions 已提取，`workspace-screen.tsx` 从 5453 降至 4270 行；下一步拆分 tab close/bulk-close lifecycle，保持 native/web/electron surface 测试分离。
