...
| -------- | -------: | -------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| 架构设计 |      8.9 | dependency-cruiser 0 违规；DaemonClient terminal 与 voice/dictation 生命周期已独立，核心降至 3920 行     | provider adapters、`workspace-screen.tsx`、`messages.ts` 仍是 4k+ 行责任中心                    |
...
| 代码质量 |      8.7 | typecheck/lint/format/test-audit/高信号 Knip/依赖边界均有门禁；terminal/voice 具备领域级契约测试         | 历史 test debt 高，核心测试文件超过 5k 行，完整 Knip unused-export 结果仍有大量噪声与真实债混合 |
| 综合     |  **9.0** | 核心安全、主要产品域 parity 与持续领域拆分均有代码级实现和精确验证                                       | 继续提升需要完成 provider/app/protocol 大模块治理和依赖专项                                     |
...
3. **P1 client/protocol 拆分（进行中）**：`daemon-client.ts` 已完成文件传输、checkout、管理命令、automation、workspace、terminal 与 voice/dictation 分域，核心降至 3920 行；继续拆 agent/connection 领域，并按 RPC domain 拆 `messages.ts`。
