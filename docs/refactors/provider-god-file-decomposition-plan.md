...
- ACP 已完成十个边界切片：tool/permission mapper、session config mapping、NDJSON transport、process runtime、terminal controller、workspace path、session update controller、foreground turn controller、command catalog 与 session config controller 分别拥有投影、纯配置映射、编解码兼容、process lifecycle、terminal ownership、路径意图、message/tool update、prompt/turn/usage/终态、slash-command wait，以及 mode/model/thinking 状态与写入；`acp-agent.ts` 继续拥有 Client/Session façade 与 process-session lifecycle，并兼容重导出原公开 API。
...
| `acp-agent.ts`                       | 1070 | `ACPAgentSession`             | `ACPAgentClient`                   | —              | —         |
...
- `acp-agent.ts` —— 保留 Client/Session façade、持久化、permission、文件/terminal façade 与 process-session lifecycle（进行中，1070 行；原公开 mapper/config/transport/process/usage/session-state 类型兼容重导出）

**验收**：server typecheck、新 mapper 5 个断言、既有 generic permission、mode/model/config 7 个、stream/compat 3 个、initialize fail-cleanup、terminal 3 个、workspace path 1 个、session update controller 的 7 个 mode/config/permission/commands/message 场景、foreground turn controller 的 3 个 prompt completion/failure/JSON-RPC diagnostic 场景、command catalog 的 2 个命令发现场景，以及 session config controller 的 11 个初始化/override/provenance/update/canonical response 场景通过；各批目标 lint 为 0 error/0 warning，真实 provider wrapper smoke 未在无凭据本地环境运行。
