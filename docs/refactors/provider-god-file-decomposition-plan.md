...
- OpenCode 已完成 façade、session、client runtime、event translator、helpers、catalog、runtime、abort coordinator 与 event-stream controller 九个边界切片；原入口为 64 行兼容 façade，native event translation、usage、permission、todo 与 sub-agent timeline 映射已独立。

## 现状

三个 provider agent 实现均直接 `implements AgentSession` / `implements AgentClient`，
**无共享基类、无 mixin、无 abstract class**。Codex Session 已收敛到 715 行；Claude Session 为 3001 行；OpenCode façade 为 64 行、Session 为 1396 行、event translator 为 1093 行、Client runtime 为 507 行。后续继续拆分 OpenCode history 与 Session orchestration。
...
| `opencode/session.ts`          | 1396 | `OpenCodeAgentSession`       | —                                  | 18             | 17        |
...
- `opencode/session.ts` —— `OpenCodeAgentSession` 与 history（已完成，1396 行；入口收敛为 64 行兼容 façade）
