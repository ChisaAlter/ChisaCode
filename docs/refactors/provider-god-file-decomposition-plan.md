...
- ACP 已完成九个边界切片：tool/permission mapper、session config、NDJSON transport、process runtime、terminal controller、workspace path、session update controller、foreground turn controller 与 command catalog 分别拥有投影、配置、编解码兼容、process lifecycle、terminal ownership、路径意图、message/tool update 路由、prompt/active turn/usage/终态，以及 slash-command snapshot/首次等待/关闭唤醒；`acp-agent.ts` 继续拥有 Client/Session、mode/config 与 process-session lifecycle 编排，并兼容重导出原公开 API。
...
| `acp-agent.ts`                       | 1517 | `ACPAgentSession`             | `ACPAgentClient`                   | —              | —         |
...
- `acp-agent.ts` —— 保留 Client/Session orchestration、mode/config、写入时序与 process-session lifecycle（进行中，1517 行；原公开 mapper/config/transport/process/usage 类型兼容重导出）

**验收**：server typecheck、新 mapper 5 个断言、既有 generic permission、mode/model/config 7 个、stream/compat 3 个、initialize fail-cleanup、terminal 3 个、workspace path 1 个、session update controller 的 7 个 mode/config/permission/commands/message 场景、foreground turn controller 的 3 个 prompt completion/failure/JSON-RPC diagnostic 场景，以及 command catalog 的 2 个立即返回/异步 update 聚焦场景通过；各批目标 lint 为 0 error/0 warning。
