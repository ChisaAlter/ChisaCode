...
- ACP 已完成七个边界切片：tool/permission mapper、session config、NDJSON transport、process runtime、terminal controller、workspace path 与 session update controller 分别拥有投影、配置、编解码兼容、process lifecycle、terminal ownership、路径意图，以及 message/tool update 状态与路由；`acp-agent.ts` 继续拥有 Client/Session、mode/config、turn 与 lifecycle 编排，并兼容重导出原公开 API。
...
| `acp-agent.ts`                       | 1752 | `ACPAgentSession`             | `ACPAgentClient`                   | —              | —         |
...
- `acp-agent.ts` —— 保留 Client/Session orchestration、mode/config、turn、写入时序、进程与 lifecycle（进行中，1752 行；原公开 mapper/config/transport/process 类型兼容重导出）

**验收**：server typecheck、3 个目标文件 lint、新 mapper 5 个断言、既有 generic permission、mode/model/config 7 个、stream/compat 3 个、initialize fail-cleanup、terminal 3 个、workspace path 1 个，以及 session update controller 的 7 个 mode/config/permission/commands/message 聚焦场景通过。
