...
- **Provider composition-first 拆分启动**：完成 Codex skills、notification parser/router、turn configuration、model catalog、launch/runtime config、client/client runtime、session、tool/delta/item/turn notification、notification/compaction state、notification timeline、sub-agent tracker、permission state/domain/controller、session event bus、user-message turn state、image attachments、history pipeline 二十五个领域切片；`turn-notification-handler.ts` 完整拥有 usage、plan result 与 textual-tool-call failure 等 turn-scoped 状态，并接管 thread/turn/plan/usage/rollback/compaction 通知。Session 从 1659 降至 1477 行。
...
- **状态**：进行中；Codex notification handler 领域已按 tool/delta/item/turn 完整拆分；下一步拆 `codex/session.ts` 的连接、恢复、模型解析与公共 Session API 编排。
