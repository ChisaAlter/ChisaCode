...
- **状态**：进行中；Codex notification、thread bootstrap、metadata 与 history state 已拆分；下一步拆连接握手和公共 Session API/命令编排。
