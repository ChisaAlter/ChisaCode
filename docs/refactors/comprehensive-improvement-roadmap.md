...
- **ACP mapping/transport 拆分**：完成三个 ACP provider 切片。`acp/tool-call-mapper.ts` 独立拥有 tool/permission 投影；`acp/session-config.ts` 独立拥有模式/模型/config 映射；`acp/ndjson-stream.ts` 独立拥有 NDJSON 编解码、非法 stdout 诊断与 stringified numeric response ID 兼容。Session 继续拥有状态、写入时序、进程、事件和 terminal 生命周期，原公开入口兼容重导出。`acp-agent.ts` 从 2860 降至 2143 行；server typecheck、目标 lint、mapper 6 个、config 7 个、transport 3 个聚焦场景通过。
