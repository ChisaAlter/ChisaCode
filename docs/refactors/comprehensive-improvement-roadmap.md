...
- **Provider composition-first 拆分启动**：完成 Codex skills/custom prompts、notification parser、turn configuration 三个领域切片；`codex/skills.ts` 承接扩展发现与策略，`codex/notifications.ts` 承接 24 类 app-server notification 的 Zod 归一化，`codex/turn-config.ts` 承接 mode preset、sandbox policy、output schema 和 `turn/start` 参数构建。原 5981 行核心文件降至 4582 行。另删除 437 行从未接线且会改变 provider 生命周期语义的 speculative `providers/base/` 基类。
...
- **状态**：进行中；采用 composition-first，Codex skills、notification parser 与 turn configuration 切片已完成，后续继续拆 event handlers、session/client 与 runtime。
