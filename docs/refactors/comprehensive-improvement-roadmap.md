...
- **Provider composition-first 拆分启动**：完成 Codex skills、notification parser/router、turn configuration、model catalog、notification/compaction state、notification timeline、sub-agent tracker、permission state/domain、image attachments、history pipeline 十三个领域切片；`context-compaction-state.ts` 管理手动触发归因、itemId 生命周期和 notification/item 双通道完成去重。原 5981 行核心文件降至 3123 行；另删除 437 行未接线 speculative 基类。
...
- **状态**：进行中；采用 composition-first，Codex skills、notification parser/router、turn configuration、models、notification/compaction state、notification timeline、sub-agent tracker、permission state/domain、image attachments 与 history pipeline 切片已完成，后续继续拆 event handlers、session/client 与 runtime。
