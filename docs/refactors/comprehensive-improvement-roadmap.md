...
- **Provider composition-first 拆分启动**：完成 Codex skills/custom prompts 与 notification parser 两个领域切片，将技能发现/策略过滤/参数展开提取到 `codex/skills.ts`，将 24 类 app-server notification 的 Zod 校验、归一化和判别联合提取到 `codex/notifications.ts`；原 5981 行核心文件降至 4799 行。另删除 437 行从未接线且会改变 provider 生命周期语义的 speculative `providers/base/` 基类，后续仅在稳定同构契约被两个 provider 验证后再考虑继承抽象。
...
- **状态**：进行中；采用 composition-first，Codex skills/custom prompts 与 notification parser 切片已完成，后续继续拆 event handlers、session/client 与 runtime。
