...
- **ACP composition-first 拆分**：完成七个 ACP provider 切片。tool/permission、session config、NDJSON transport、process runtime、terminal 与 workspace path 已分域；`acp/session-update-controller.ts` 新增独立消息分片组装、tool snapshot 生命周期、update 路由与取消态合成，mode/config/session-info/commands 通过窄回调继续由 Session 持有。原公开入口和私有 `translateSessionUpdate` 测试契约保持兼容，wrapper smoke 的 tool 证据改为 timeline 行为而非 Session 私有 map；`acp-agent.ts` 从 2860 降至 1752 行。server typecheck、3 个目标文件 lint 与 7 个 update/permission/commands/message 聚焦场景通过。
