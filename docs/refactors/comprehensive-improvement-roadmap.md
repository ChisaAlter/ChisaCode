...
- **ACP composition-first 拆分**：完成八个 ACP provider 切片。tool/permission、session config、NDJSON transport、process runtime、terminal 与 workspace path 已分域；`acp/session-update-controller.ts` 独立拥有消息分片、tool snapshot、update 路由与取消态合成，`acp/foreground-turn-controller.ts` 独立拥有 prompt 派发、active turn、usage、用户回显抑制、bootstrap thread 事件、终态与 process-exit failure。每回合 usage 会显式重置，进程退出、关闭或后续回合替换后的迟到 prompt 结果不再污染终态；测试改用重叠回合拒绝与完成/失败后可重试等公开行为，不再读取私有 turn 状态。原公开入口继续兼容重导出，`acp-agent.ts` 从 2860 降至 1575 行；server typecheck、3 个目标文件 lint 与 3 个 prompt completion/failure/JSON-RPC diagnostic 聚焦场景通过。
