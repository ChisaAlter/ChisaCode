...
- **ACP tool/config 映射拆分**：完成两个 ACP provider 切片。`acp/tool-call-mapper.ts` 独立拥有增量 tool snapshot、plan/timeline/detail、permission request/option 与 raw payload 解析；`acp/session-config.ts` 独立拥有模式/模型目录派生、selection 校验、select option 展平和 provider mode writer 契约。Session 继续拥有状态、写入时序、事件编排和 terminal 生命周期，原公开入口兼容重导出。`acp-agent.ts` 从 2860 降至 2241 行；server typecheck、目标 lint、mapper 6 个契约场景与 config 7 个聚焦断言通过。
