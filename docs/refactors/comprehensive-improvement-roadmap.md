...
- **ACP tool-call/permission 映射拆分**：完成首个 ACP provider 切片。将增量 tool snapshot 合并、plan/timeline/detail 映射、permission request/option 选择与 raw payload 解析提取到 `acp/tool-call-mapper.ts`；Session 继续拥有状态、事件编排和 terminal 生命周期，原 `ACPToolSnapshot` 入口兼容重导出。`acp-agent.ts` 从 2860 降至 2446 行；server typecheck、3 个目标文件 lint、新 mapper 5 个断言与既有 permission 透传场景通过。
