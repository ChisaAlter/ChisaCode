...
### Provider 可用性探测风暴与 error 状态挡模型列表（2026-08-12 登记，2026-08-13 阶段一完成）
...
- **状态**：阶段一完成（2026-08-13）。B-app / 服务端守卫 A+B / E 已落地；聚焦 vitest + typecheck + lint 绿。**打包桌面实机 PASS**（`npx tsx packages/app/e2e/desktop-provider-probe-gate.script.ts`，隔离 home + mock/mock-slow）：开选择器 unscoped refresh=0；Mock 4 模型可选、触发器 `Five minute stream`、无「请选择模型」；Mock Slow 超时后显示 error 空态+重试，点重试回到 loading（定向 force）。证据 `.omo/evidence/desktop-provider-probe-gate-2026-08-13T02-48-23.md` + 4 张截图。阶段二（探测与 MCP 解耦）待立项。原复现证据：`provider-snapshot-manager.ts:766` 超时文案 + 真实 daemon 日志 `Failed to check provider availability` 批量超时 + `_x.ai/mcp/init_progress {total:5,connected:0}`
