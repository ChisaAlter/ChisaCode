...
- **状态**：进行中；三个 provider 均已建立稳定 façade/client/session 边界，OpenCode turn execution/sub-agent tracking/translator/history/permission/MCP/event bus/runtime/lifecycle 已独立。下一步继续按事件领域收敛 translator。
