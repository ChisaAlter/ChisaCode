...
- **App workspace header view 拆分**：完成。新增 `workspace-header.tsx`，统一拥有 workspace menu、responsive title、desktop tab presentation、mobile scripts、explorer/environment toggles、静态 icon 与全部 header 专用样式；同步删除 8 个纯静态 icon 透传 props。新视图文件为 610 行，`workspace-screen.tsx` 从 2592 降至 1959 行；App typecheck 与 2 个目标文件 lint 通过，下一步评估 center-column render shell。
