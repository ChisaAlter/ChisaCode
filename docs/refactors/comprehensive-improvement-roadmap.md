...
- **Provider composition-first 拆分启动**：完成 Codex skills、notification parser/router、turn configuration、model catalog、notification/compaction state、notification timeline、sub-agent tracker、permission state/domain/controller、image attachments、history pipeline 十四个领域切片；`permission-controller.ts` 承接 command/file/question/plan 请求校验、pending 生命周期、响应决策、deny/question timeline 与 plan follow-up，session 只注入 cwd、emit 和 plan-mode 回调。原 5981 行核心文件降至 2817 行；另删除 437 行未接线 speculative 基类。
...
- **状态**：进行中；采用 composition-first，Codex skills、notification parser/router、turn configuration、models、notification/compaction state、notification timeline、sub-agent tracker、permission state/domain/controller、image attachments 与 history pipeline 切片已完成，后续继续拆 turn/item handlers、session/client 与 runtime。
