...
- **ACP composition-first 拆分**：完成十个 ACP provider 切片。tool/permission、session config mapping、NDJSON transport、process runtime、terminal/path、session update、foreground turn 与 command catalog 已分域；新增 `acp/session-config-controller.ts` 完整拥有 mode/model/thinking 状态、启动 override、provider writer、config response 规范化和 session/config update。Session 的配置 API 收敛为薄委托，`SessionStateResponse` 继续从原入口兼容重导出；同时记录 mode 目录来源，config-derived mode 不再被误判为原生 `availableModes` 并错误调用 `setSessionMode`。`acp-agent.ts` 从 2860 降至 1070 行；server typecheck、3 个目标文件 lint 与 11 个配置写入/更新聚焦场景通过。
