...
- **Provider composition-first 拆分启动**：完成 Codex skills/custom prompts 首个领域切片，将技能发现、front matter、策略过滤与参数展开提取到 `codex/skills.ts`，原 5981 行核心文件降至 5656 行；删除 437 行从未接线且会改变 provider 生命周期语义的 speculative `providers/base/` 基类，后续仅在稳定同构契约被两个 provider 验证后再考虑继承抽象。
...
- **状态**：进行中；采用 composition-first，Codex skills/custom prompts 切片已完成，后续继续拆 event routing、session/client 与 runtime。
