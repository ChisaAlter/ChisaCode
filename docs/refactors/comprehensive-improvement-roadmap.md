...
- **状态**：进行中；三个 provider 均已建立稳定 façade/client/session 边界，OpenCode 主事件路由完成收敛；Claude permission/options/persisted history/message translation/rewind state 已独立。下一步拆分 Claude session lifecycle。
