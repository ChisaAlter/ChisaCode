...
- **状态**：P0-P2 + S9 完成（S1-S9，分支 `fix/cindy-integration-hardening`，9 个提交）。所有 critical/high 缺陷修复：协议 exports/gate、snapshot 注入+workspace 绑定、SSH 加固、临时 index、team spawn/队列/回收、goal 取消传播、GoalStatus 枚举+leak、model-catalog/定价、guard 接入 CI+i18n 校验+删副本。S10（消息渲染补全）/S11（同会话切换）属产品功能扩展，单独立项排期。后续跟踪项：team 全局 worker 上限、goal usedTools 跨 controller 重置竞争、learn distill cancel。
