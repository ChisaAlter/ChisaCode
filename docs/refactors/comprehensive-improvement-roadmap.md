...
- **Provider composition-first 拆分启动**：完成 Codex skills、notification parser/router、turn configuration、model catalog、launch/runtime config、client/client runtime、session/thread bootstrap/session metadata/session history、tool/delta/item/turn notification、notification/compaction state、notification timeline、sub-agent tracker、permission state/domain/controller、session event bus、user-message turn state、image attachments、history pipeline 三十一个领域切片；`session-history.ts` 完整拥有 persisted history pending/entries、user-message 索引重建与单次 drain，`session-connection.ts` 完整拥有 client、并发 connect 去重、initialize handshake、失败清理与 close 竞态，`session-commands.ts` 完整拥有 slash-command 解析、custom prompt/skill 展开、命令目录及 `/compact`/`/goal` 编排，`session-runtime.ts` 完整拥有 config/mode/feature/service tier、runtime info cache 与 persistence metadata。Session 从 907 降至 828 行，runtime 198 行。
...
- **状态**：进行中；Codex notification、thread bootstrap、metadata、history state、连接生命周期、命令编排与 runtime/persistence 已拆分；下一步拆 turn execution/rewind lifecycle。
