...
- **ACP mapping/process 拆分**：完成四个 ACP provider 切片。tool/permission 投影、模式/模型/config、NDJSON transport 分别独立；新增 `acp/process-runtime.ts` 统一 command resolution、环境、spawn、initialize timeout/error 与进程终止，Client probe 和 live Session 共用同一失败清理语义。Session 继续拥有业务状态、写入时序、事件和 terminal 生命周期，原公开入口兼容重导出。`acp-agent.ts` 从 2860 降至 2001 行；server typecheck、目标 lint、既有 mapper/config/transport 场景与 initialize timeout cleanup 聚焦测试通过。
...
- **状态**：进行中；三个 provider 均已建立稳定 façade/client/session 边界，OpenCode 主事件路由完成收敛；Claude permission/options/persisted history/message translation/rewind/query lifecycle 已独立。下一步评估 Claude 剩余 session identity/runtime orchestration，并转向 App/CLI/MCP 产品能力一致性。
