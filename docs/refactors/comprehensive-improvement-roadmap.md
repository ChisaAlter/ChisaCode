...
- **Provider composition-first 拆分启动**：完成 Codex skills、notification parser、turn configuration、model catalog、notification stream state/timeline、sub-agent tracker、permission state/domain、image attachments、history pipeline 十一个领域切片；`notification-timeline.ts` 承接 shell wrapper/base64 delta、patch 兼容解析、edit 完整性和 terminal timeline 映射，旧 patch mapper 导出保持兼容。原 5981 行核心文件降至 3224 行；另删除 437 行未接线 speculative 基类。
...
- **状态**：进行中；采用 composition-first，Codex skills、notifications、turn configuration、models、stream state/timeline、sub-agent tracker、permission state/domain、image attachments 与 history pipeline 切片已完成，后续继续拆 event handlers、session/client 与 runtime。
