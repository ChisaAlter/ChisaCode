...
- **ACP composition-first 拆分**：完成六个 ACP provider 切片。tool/permission、session config、NDJSON transport、process runtime 已分域；`acp/terminal-controller.ts` 独立拥有 terminal 子进程、输出截断、exit waiter 与关闭清理，`acp/workspace-path.ts` 供 fs/terminal 共享意图路径边界，并修正 `..cache` 等合法目录误拒绝。Session 保留业务状态、写入时序和事件编排，原公开入口兼容重导出。`acp-agent.ts` 从 2860 降至 1866 行；server typecheck、目标 lint、既有 terminal 3 个、路径边界 1 个及前序聚焦场景通过。
...
- **状态**：进行中；三个 provider 均已建立稳定 façade/client/session 边界，OpenCode 主事件路由完成收敛；Claude permission/options/history/rewind/query lifecycle 与 Pi extension history/session events 已独立。下一步收敛 ACP 剩余 session 编排、Pi runtime/session lifecycle 和 Claude identity/runtime orchestration。
