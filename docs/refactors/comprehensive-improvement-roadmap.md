...
- **问题**：侧边栏「移除项目」操作把 workspace group key 写入持久化的 `hiddenSessionGroupKeysByServerId`（`sidebar-order-store.ts`，persist key `sidebar-project-workspace-order`），但该黑名单只增不减——`setSessionGroupHidden(..., false)` 无任何调用点，reconcile effect 只对齐 order 不清理 hidden keys，空态无恢复入口。一旦用户对某个活跃 workspace 按过「移除项目」，该 group key 被永久写入；后续该 workspace 重新出现 agent（新建对话落回同 cwd、重新打开项目）时，`scope: "active"` 返回的 agent 全部落入被隐藏组 → `workspaceGroups.length === 0` → `sidebar-session-list.tsx` 渲染 `noSessions` 空态，即使服务端 `ls` 返回未归档 agent。实测桌面端 `hiddenSessionGroupKeysByServerId["srv_..."]` 含 `c:/ai/chisacode`，服务端 10 个未归档 agent 全被该隐藏 key 吞掉，侧边栏完全空白。这不是加载/订阅/归档/provider 过滤问题，是黑名单设计的单向性缺陷。
- **影响包**：`packages/app`（stores/sidebar-order-store、components/sidebar-session-list、i18n）。
- **修复**：三层。1) `sidebar-order-store` 新增 `setHiddenSessionGroupKeys`（批量替换，带 no-op 守卫避免 effect 循环）与 `clearHiddenSessionGroupKeys`（清空某 server 全部隐藏 key）两个 action。2) `sidebar-session-list.tsx` 的 reconcile useEffect 追加：当某被隐藏的 group 在当前 `activitySortedGroups` 中重新出现 agent 时，自动从黑名单移除（自愈，永不永久空白）。3) 空态安全网：`visibleAgents` 非空但 `workspaceGroups` 因全隐藏为空时，退回显示所有非 pinned 组，防止 effect 首帧前死锁。4) 空态加「显示已隐藏的项目」按钮（i18n `sidebar.showHiddenProjects` 中英），调用 `clearHiddenSessionGroupKeys`，给用户主动恢复入口。同步 `.js` 编译副本（Vitest `resolve.extensions` 中 `.js` 优先于 `.ts`，未同步会导致测试解析到旧 action）。
- **验收**：`sidebar-order-store.test.ts` 5 断言全绿（含新增 `setHiddenSessionGroupKeys` 去重/no-op、`clearHiddenSessionGroupKeys`、reconcile 自愈回归测试）。
- **状态**：完成。
