...
- **Provider composition-first 拆分启动**：完成 Codex skills、notification parser/router、turn configuration、model catalog、notification stream state/timeline、sub-agent tracker、permission state/domain、image attachments、history pipeline 十二个领域切片；`notification-router.ts` 集中 schema parse、delta 判别和 17 类通知分派，session 入口改为单行委托。原 5981 行核心文件降至 3173 行；另删除 437 行未接线 speculative 基类。
...
- **状态**：进行中；采用 composition-first，Codex skills、notification parser/router、turn configuration、models、stream state/timeline、sub-agent tracker、permission state/domain、image attachments 与 history pipeline 切片已完成，后续继续拆 event handlers、session/client 与 runtime。
