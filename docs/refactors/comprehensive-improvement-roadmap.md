...
- **状态**：进行中；Codex Session 已收敛为 715 行 orchestrator；`rewind.ts` 已承担 fork/rollback 核心语义，不再增加重复包装。下一步转向 5185 行 Claude provider 的 SDK pump/tool/history 边界。
