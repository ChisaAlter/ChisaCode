...
- **状态**：进行中；三个 provider 均已建立稳定 façade/client/session 边界，OpenCode 主事件路由完成收敛；Claude permission/options/history/rewind/query lifecycle 与 Pi extension history 已独立。下一步收敛 ACP 剩余 session 编排、Pi runtime/event routing 和 Claude identity/runtime orchestration。
