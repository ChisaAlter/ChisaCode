...
- **Provider composition-first 拆分启动**：完成 Codex skills、notification parser、turn configuration、model catalog、notification stream state、sub-agent tracker、permission state/domain、image attachments 九个领域切片；`image-attachments.ts` 集中 base64/data URI 归一化、私有临时文件写入、历史图片 materialize 与 TTL 清理，旧 cleanup 导出保持兼容。原 5981 行核心文件降至 3775 行；另删除 437 行未接线 speculative 基类。
...
- **状态**：进行中；采用 composition-first，Codex skills、notifications、turn configuration、models、stream state、sub-agent tracker、permission state/domain 与 image attachments 切片已完成，后续继续拆 event handlers、session/client 与 runtime。
