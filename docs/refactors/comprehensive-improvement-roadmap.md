...
- **ACP composition-first 拆分**：完成九个 ACP provider 切片。tool/permission、session config、NDJSON transport、process runtime、terminal 与 workspace path 已分域；session update controller 独立拥有消息/tool update，foreground turn controller 独立拥有 prompt/active turn/usage/终态，`acp/command-catalog.ts` 独立拥有 slash-command snapshot、可选首次 update 等待、超时与关闭唤醒。每回合 usage 显式重置，迟到 prompt 结果不再污染终态，命令目录等待行为保持兼容；测试改用公开行为而非私有状态。原公开入口继续兼容重导出，`acp-agent.ts` 从 2860 降至 1517 行；server typecheck、2 个目标文件 lint 与 2 个命令发现聚焦场景通过。
