...
- **Provider composition-first 拆分启动**：完成 Codex skills、notification parser/router、turn configuration、model catalog、launch/runtime config、client/client runtime、session、tool/delta/item notification、notification/compaction state、notification timeline、sub-agent tracker、permission state/domain/controller、session event bus、user-message turn state、image attachments、history pipeline 二十四个领域切片；`item-notification-handler.ts` 接管 started/completed、stream suffix、compaction item、user-message 去重、sub-agent child item 与 lifecycle 去重。Session 从 1916 降至 1659 行，下一步拆 turn lifecycle。
...
- **状态**：进行中；Codex client/session/tool/delta/item notification 边界已落地；下一步拆 `codex/session.ts` 内 turn lifecycle 与连接/恢复编排。
