...
- **Provider composition-first 拆分启动**：完成 Codex skills、notification parser、turn configuration、model catalog、notification stream state、sub-agent tracker 六个领域切片；`notification-stream-state.ts` 管理流式缓冲与生命周期去重，`sub-agent-tracker.ts` 管理 child thread 映射、子时间线排序和父工具调用状态重建，事件发送仍留在 session。原 5981 行核心文件降至 4223 行；另删除 437 行未接线 speculative 基类。
...
- **状态**：进行中；采用 composition-first，Codex skills、notifications、turn configuration、models、stream state 与 sub-agent tracker 切片已完成，后续继续拆 event handlers、session/client 与 runtime。
