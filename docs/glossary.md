...
- **Daemon** — Local ChisaCode server process; identified by `serverId`. UI: "Daemon" (system contexts only). Code: `serverId` in `ServerInfoStatusPayloadSchema` (`packages/protocol/src/messages.ts:1936`), `DaemonClient` (`packages/client/src/daemon-client.ts`).
...
- **Worktree** — ChisaCode-managed git worktree (`~/.chisacode/worktrees/{name}`); also a `workspaceKind` value. UI: CLI + `chisacode.json` keys (`worktree.setup`, `worktree.teardown`) only. Code: `ProjectCheckoutLiteGitChisaCodePayload` (`packages/protocol/src/messages.ts:2092`); CLI `chisacode worktree` (`packages/cli/src/commands/worktree/index.ts:8`). Forbidden: "Checkout" as a synonym.
...
- **Provider** — Agent backend (Claude, Codex, OpenCode, Pi, Kimi Code, Grok Build, DeepSeek Harness (`dsh`), or a configured custom provider). UI: "Provider". Code: `ProviderSnapshotEntry`.
...
- **Conflict** — Two distinct senses; do NOT use the bare word in UI copy without qualifying which: (a) **stale-write conflict** on `chisacode.json` ("Config changed on disk", code `stale_project_config`, `packages/app/src/screens/project-settings-screen.tsx:593`); (b) **git merge conflict** (no current UI string).
