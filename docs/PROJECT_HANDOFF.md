...
ChisaCode is a local-first control surface for AI coding agents. A local daemon manages agent processes and state, clients connect over WebSocket, and optional relay support allows encrypted remote access. The workspace is an npm monorepo pinned to Node 22.20.0.
