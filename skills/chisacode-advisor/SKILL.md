...
Read the **chisacode** skill — provider for the advisor comes from orchestration preferences unless the user names one.
...
- `/chisacode-advisor /unslop` → advisor runs `/unslop` on the current diff.
- `/chisacode-advisor /unslop-risk` → advisor does an unslop-risk review.
- `/chisacode-advisor $diagnose this build failure` → advisor invokes `/diagnose`.
...
Create the advisor agent via ChisaCode with a `[Advisor] <topic>` title and the briefing as the initial prompt. Wait for it to finish. Read its response. Synthesize for the user — the advisor's verdict + your recommendation.
